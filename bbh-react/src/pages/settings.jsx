...
<body class="h-screen flex flex-col bg-gray-900 text-white relative">
    <!-- HEADER -->
    ...
    <!-- CONTENU PRINCIPAL -->
    <main
        class="flex flex-1 flex-col lg:flex-row mt-6 max-w-7xl mx-auto w-full bg-cyan-900 mb-6 rounded-lg shadow-lg border border-black overflow-hidden">

        <!-- SIDEBARMENU settings -->
        <aside
            class="w-full lg:w-64 bg-bbh-sidebarcont border-b-2 lg:border-b-0 lg:border-r border-gray-700 p-4 lg:p-5 flex-shrink-0 flex flex-col lg:flex-col justify-start overflow-x-auto lg:overflow-y-auto scrollbar-hide"
            aria-label="Sommaire des options">
            <h2 class="text-bbh-accent text-lg font-semibold mb-3 lg:mb-4 text-center hidden sm:block ">Paramètres</h2>
            <ul class="flex lg:flex-col gap-2 sm:gap-3 whitespace-nowrap justify-start lg:justify-normal">
                <li><button
                        class="tab-btn w-full text-left py-2 px-3 rounded hover:bg-bbh-accent hover:text-cyan-400 cursor-pointer"
                        data-tab="profil">> Profil</button></li>
                <li><button
                        class="tab-btn w-full text-left py-2 px-3 rounded hover:bg-bbh-accent hover:text-cyan-400 cursor-pointer"
                        data-tab="securite">> Sécurité</button></li>
                <li><button
                        class="tab-btn w-full text-left py-2 px-3 rounded hover:bg-bbh-accent hover:text-cyan-400 cursor-pointer"
                        data-tab="notifications">> Notifications</button></li>
                <li><button
                        class="tab-btn w-full text-left py-2 px-3 rounded hover:bg-bbh-accent hover:text-cyan-400 cursor-pointer"
                        data-tab="apparence">> Apparence</button></li>
                <li><button
                        class="tab-btn w-full text-left py-2 px-3 rounded hover:bg-bbh-accent hover:text-cyan-400 cursor-pointer"
                        data-tab="avance">> Avancé</button></li>
            </ul>
        </aside>
        ...
        <section class="flex-1 overflow-y-auto p-4 sm:p-6 lg:p-8 space-y-6 sm:space-y-8">
            ...
            <div id="profil" class="tab-content">
                <h3 class="text-bbh-accent text-lg sm:text-xl font-semibold mb-3 sm:mb-4">Profil utilisateur</h3>
                <div class="space-y-3 sm:space-y-4">
                    <div>
                        <label for="username" class="block mb-1 font-medium text-sm sm:text-base">Identifiant</label>
                        <input type="text" id="username" placeholder="Votre identifiant" required minlength="3"
                            maxlength="40" autocomplete="off"
                            class="w-full bg-bbh-dark border border-gray-700 rounded p-2 text-sm sm:text-base focus:border-bbh-accent focus:outline-none">
                    </div>
                    <div>
                        <label for="email" class="block mb-1 font-medium text-sm sm:text-base">Adresse e-mail</label>
                        <input type="email" id="email" placeholder="votre@adresse.email" required minlength="8"
                            maxlength="40"
                            class="w-full bg-bbh-dark border border-gray-700 rounded p-2 text-sm sm:text-base focus:border-bbh-accent focus:outline-none">
                    </div>
                    <div>
                        <label for="phonenumber" class="block mb-1 font-medium text-sm sm:text-base">Numéro de
                            téléphone</label>
                        <input type="tel" id="phonenumber" placeholder="+33 6 12 34 56 78"
                            class="w-full bg-bbh-dark border border-gray-700 rounded p-2 text-sm sm:text-base focus:border-bbh-accent focus:outline-none">
                    </div>
                    <button id="settingsForm"
                        class="bg-cyan-400 text-black font-semibold px-4 py-2 rounded text-sm sm:text-base hover:bg-sky-600 transition cursor-pointer">Enregistrer</button>
                </div>
            </div>
            <!-- SÉCURITÉ -->
            <div id="securite" class="tab-content hidden">
                <h3 class="text-bbh-accent text-xl font-semibold mb-4">Sécurité du compte</h3>
                <div class="space-y-4">
                    <div>
                        <label for="NouveauPass" class="block mb-1 font-medium">Nouveau mot de passe</label>
                        <input type="password" id="masterPassword" placeholder="••••••••••••••••"
                            class="w-full bg-bbh-dark border border-gray-700 rounded p-2 focus:border-bbh-accent focus:outline-none">
                    </div>
                    <!-- Bloc double authentification -->
                    <div class="bg-bbh-medium border border-gray-700 rounded-lg p-6 shadow-lg space-y-4">
                        <div class="flex items-center gap-3 mb-2">
                            <img src="images/BBH-45x45.png" alt="Icône sécurité" class="h-8 w-8">
                            <h4 class="text-lg font-semibold text-bbh-accent">Double authentification (2FA)</h4>
                        </div>

                        <p class="text-sm text-gray-300 leading-relaxed">
                            La double authentification ajoute une couche de sécurité supplémentaire à votre compte.
                            En plus de votre mot de passe, un code temporaire à usage unique vous sera demandé lors de
                            chaque connexion.
                        </p>

                        <div class="flex items-center gap-2">
                            <input type="checkbox" id="twofa" class="accent-bbh-accent h-4 w-4">
                            <label for="twofa" class="text-sm">Activer la double authentification (2FA)</label>
                        </div>

                        <!-- Bloc dynamique d’activation -->
                        <div id="twofa-setup"
                            class="hidden mt-4 bg-bbh-dark border border-gray-700 rounded p-4 space-y-4">
                            <p class="text-sm text-gray-300">
                                Scannez le code QR ci-dessous avec votre application d’authentification, puis saisissez
                                le code généré :
                            </p>

                            <!-- QR Code -->
                            <div class="flex justify-center">
                                <canvas id="qrCodeCanvas" class="rounded-lg shadow-md bg-white p-2"></canvas>
                            </div>

                            <!-- Code généré -->
                            <div class="text-center">
                                <p class="text-gray-400 text-sm mb-1">Code actuel :</p>
                                <div id="otpCode" class="text-3xl font-mono tracking-widest text-bbh-accent">••••••
                                </div>
                                <div class="text-xs text-gray-400 mt-1">Régénération dans <span id="otpTimer">30</span>
                                    s</div>
                            </div>

                            <input type="text" placeholder="Entrez votre code à 6 chiffres"
                                class="w-full bg-bbh-medium border border-gray-700 rounded p-2 text-center focus:border-bbh-accent focus:outline-none mb-3">

                            <button
                                class="bg-cyan-600 text-bbh-dark font-semibold px-4 py-2 rounded hover:bg-cyan-400 transition w-full cursor-pointer">Vérifier
                                et activer</button>
                        </div>
                    </div>
                </div>
                <button
                    class="bg-sky-500 text-black font-semibold px-4 py-2 rounded hover:bg-cyan-400 transition cursor-pointer mt-2">Mettre
                    à jour</button>
            </div>
            </div>
            ...
            <div id="notifications" class="tab-content hidden">
                <h3 class="text-bbh-accent text-xl font-semibold mb-4">Préférences de notifications</h3>
                <div class="space-y-3">
                    <label class="flex items-center gap-2"><input type="checkbox" checked
                            class="accent-bbh-accent h-4 w-4">Alerte de connexion</label>
                    <label class="flex items-center gap-2"><input type="checkbox"
                            class="accent-bbh-accent h-4 w-4">Notifications de sécurité</label>
                    <label class="flex items-center gap-2"><input type="checkbox" checked
                            class="accent-bbh-accent h-4 w-4">Nouveautés & mises à jour</label>
                </div>
                <button
                    class="bg-sky-500 text-black font-semibold px-4 py-2 rounded hover:bg-cyan-400 transition cursor-pointer mt-2">Sauvegarder</button>
            </div>

            <!-- APPARENCE -->
            <div id="apparence" class="tab-content hidden">
                <h3 class="text-bbh-accent text-xl font-semibold mb-4">Thème du site</h3>
                <p>Choisissez entre le thème sombre (par défaut) et le thème clair.</p>
                <div class="flex gap-4 mt-4">
                    <div data-theme="dark"
                        class="theme-box dark w-24 h-16 rounded-lg border-2 border-bbh-accent cursor-pointer active:ring-2 active:ring-bbh-accent">
                    </div>
                    <div data-theme="light"
                        class="theme-box light w-24 h-16 rounded-lg border-2 border-gray-400 cursor-pointer bg-gray-100">
                    </div>
                </div>
                <button
                    class="bg-sky-500 text-black font-semibold px-4 py-2 rounded hover:bg-cyan-400 transition cursor-pointer mt-2">Appliquer</button>
            </div>

            <!-- AVANCÉ -->
            <div id="avance" class="tab-content hidden">
                <h3 class="text-bbh-accent text-xl font-semibold mb-4">Outils avancés (Administrateur)</h3>
                <p>Gestion des utilisateurs, audit de sécurité, et logs système.</p>
                <button
                    class="bg-sky-500 text-black font-semibold px-4 py-2 rounded hover:bg-cyan-400 transition cursor-pointer mt-2">Accéder</button>
            </div>
        </section>
    </main>
